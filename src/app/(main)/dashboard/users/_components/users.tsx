...
export function Users({ users, lookups }: { users: UserRow[]; lookups: UsersLookups }) {
  ...
  const filterOptions = React.useMemo(() => {
    const mergeOptions = (base: string[], values: string[]) => [
      "Tất cả",
      ...Array.from(new Set([...base.filter((item) => item !== "Tất cả"), ...values.filter(Boolean)])),
    ];
    ...
    return {
      ...
      workspace: mergeOptions(
        filters.workspace,
        users.flatMap((user) => user.workspace),
      ),
    };
  }, [users]);
  ...
  return (
    <Card>
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        <CardTitle className="text-xl leading-none">Nhân sự</CardTitle>
        <CardDescription className="max-w-[400px] leading-snug">
          Quản lý các thành viên trong tổ chức và quyền truy cập của họ
        </CardDescription>
        <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
          <InputGroup className="h-7 w-full md:w-64">
            ...
            <InputGroupAddon align="inline-end">
              <Kbd className="h-4 text-[10px]">tìm</Kbd>
            </InputGroupAddon>
          </InputGroup>
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        ...
        <div className="flex items-center justify-between gap-3 px-4">
          <div className="text-muted-foreground text-sm tabular-nums">{selectedCount} đã chọn</div>

          <Tabs defaultValue="list">
            <TabsList>
              <TabsTrigger value="list" aria-label="Chế độ danh sách">
                <Rows3 />
              </TabsTrigger>
              <TabsTrigger value="grid" aria-label="Chế độ lưới">
                <Grid />
              </TabsTrigger>
            </TabsList>
          </Tabs>
      ...
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tài khoản "{deleteTarget?.username}" sẽ bị xóa. Hồ sơ nhân sự sẽ bị xóa nếu chưa có ràng buộc dữ liệu; nếu
              không, tài khoản sẽ chuyển sang trạng thái không hoạt động.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
